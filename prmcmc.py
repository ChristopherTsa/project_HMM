...
class ParticleGibbsRolling(mcmc.GenericGibbs):
    ...
    def update_theta(self, theta, x):
        """
        Met à jour les paramètres statiques theta conditionnellement aux états x et aux observations y.

        Arguments :
        - theta : dict
            Les paramètres statiques actuels (mu, phi, sigma_eta2, xi, sigma_u2).
        - x : array
            Les états latents (trajectoires) échantillonnés.

        Retourne :
        - theta : dict
            Les paramètres statiques mis à jour.
        """
        y = self.data  # Observations (y1 : rendements, y2 : log-volatilité)

        # Mise à jour de mu
        mu_prior_mean, mu_prior_var = 0.0, 10.0**2
        n = len(x)
        x_mean = np.mean(x)
        posterior_mean = (mu_prior_mean / mu_prior_var + n * x_mean / self.var_eta) / \
                        (1 / mu_prior_var + n / self.var_eta)
        posterior_var = 1 / (1 / mu_prior_var + n / self.var_eta)
        theta['mu'] = np.random.normal(posterior_mean, np.sqrt(posterior_var))

        # Mise à jour de phi
        beta_a, beta_b = 20, 1.5
        phi_transformed = (theta['phi'] + 1) / 2  # Transformation pour [0, 1]
        sum_phi = np.sum((x[1:] - theta['mu']) * (x[:-1] - theta['mu']))
        sum_sq_phi = np.sum((x[:-1] - theta['mu'])**2)
        posterior_a = beta_a + 0.5 * (n - 1)
        posterior_b = beta_b + 0.5 * (sum_sq_phi - (sum_phi**2) / sum_sq_phi)
        phi_transformed = np.random.beta(posterior_a, posterior_b)
        theta['phi'] = 2 * phi_transformed - 1  # Retour à [-1, 1]

        # Mise à jour de sigma_eta2
        ig_a, ig_b = 5 / 2, 0.05 / 2
        sum_eta = np.sum((x[1:] - theta['mu'] - theta['phi'] * (x[:-1] - theta['mu']))**2)
        posterior_a = ig_a + (n - 1) / 2
        posterior_b = ig_b + 0.5 * sum_eta
        theta['sigma_eta2'] = 1 / np.random.gamma(posterior_a, 1 / posterior_b)

        # Mise à jour de xi
        xi_prior_mean, xi_prior_var = 0.0, 5.0**2
        residuals = y[:, 1] - x  # y2 - x
        posterior_mean = (xi_prior_mean / xi_prior_var + np.sum(residuals) / theta['var_u']) / \
                        (1 / xi_prior_var + n / theta['var_u'])
        posterior_var = 1 / (1 / xi_prior_var + n / theta['var_u'])
        theta['xi'] = np.random.normal(posterior_mean, np.sqrt(posterior_var))

        # Mise à jour de sigma_u2
        ig_a, ig_b = 5 / 2, 0.05 / 2
        sum_u = np.sum((y[:, 1] - x - theta['xi'])**2)
        posterior_a = ig_a + n / 2
        posterior_b = ig_b + 0.5 * sum_u
        theta['sigma_u2'] = 1 / np.random.gamma(posterior_a, 1 / posterior_b)
